    ...
    void build_tree(const std::vector<std::vector<std::string>> &input)
    {
        ...
        for (int i = 0; i < input.size(); i++)
        {
            ...
            for (int j = 0; j < values.size(); j++) // each element in selected string 
            {
                ...
                // Check if the current value is already an edge
...
found = false;
    ...
    {
        return (node_counter/2); // node_counter count for every connection made eg. subtree to edgenode 
